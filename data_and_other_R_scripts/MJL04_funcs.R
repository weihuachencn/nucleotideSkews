...
## -- Models can be : Chen, or Model08 from Martin; or Model 07 from Martin as well  --
dMuCalc <- function( type, data = dat, iodata = "new", model = "Model08" ){
    ...
    if( type != "i" ){
        ...
        } else if (model == "Chen" | model == "Model07") {
          # -- simplest model --
          paste("Using simplest model\n") ;
          dMu.gc.coding <- ( gc.le - gc.la ) / 2;
        } else if( model == "Model06" ){
            # -- Model06 --
            paste("Using Model06\n") ;
...
zAAcalcM08 <- function( dMu.xs, dMu.4s, dMu.io, z.4s ){
    ...
    ### PROBLEM: gc.xx * f1.gc can be > 1 (which shouldn't happen)????
