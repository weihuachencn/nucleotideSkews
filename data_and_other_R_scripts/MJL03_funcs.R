...
## -- Models can be : Chen, or Model07 from Martin ; March 10, 2015 --
## -- but in current position, both models produce identical results ...
dMuCalc <- function( type, data = dat, iodata = "new", model = "Chen" ){
    ...
    ## -- here i = interoperonic
    ##         1, 2, 4 == codon positions...
    if( type != "i" ){
        ## -- gc --
...
## -- last modified : May 18, 2014 --
## -- last modified : March 10, 2015 --> small-N limit (Bulmer)
##
...
zAAcalcM07 <- function( dMu.xs, dMu.4s, dMu.io, z.4s ){
    ## ----------------- GC -------------------
    ## -- using Martin's Model07 --
    ...
    fit <- nls( dMu.trans ~ f1 * ( gc.le - gc.la ) / 2 ,
    ...
    z.1s.le.M6.gc <- (gc.le * f1.gc - dMu.trans) * 2 / ( (1 + dMu.trans) * (1 - dMu.trans) ) ;
    z.1s.la.M6.gc <- (gc.la * f1.gc + dMu.trans) * 2 / ( (1 + dMu.trans) * (1 - dMu.trans) ) ;
    ...
    ## -- fit
    fit <- nls( dMu.trans ~ f1 * ( at.le - at.la ) / 2 ,
                start = list( f1 = 0.4 ), trace=TRUE, na.action="na.omit" ) ;
    ...
    # selection at 1s sites (Eq. 15 & 16):
    z.1s.le.M6.at <- (at.le * f1.at - dMu.trans) * 2 / ( (1 + dMu.trans) * (1 - dMu.trans) ) ;
    z.1s.la.M6.at <- (at.la * f1.at + dMu.trans) * 2 / ( (1 + dMu.trans) * (1 - dMu.trans) ) ;
...
## -- March 10, 2015: small-N limit (Bulmer) in Model07
##
## -- input :
...
zCalc <- function( dMu, dMuIO = dMu.io, model = "Chen" ){
    ...
    if (model == "Model07") {
        # -- Model07 --
        results[[ "GC.le" ]] <- ( dMu[[ "raw.GC.le" ]] - dMu.GC ) * 2 / ( (1 + dMu.GC) * (1 - dMu.GC) ) ;
        results[[ "GC.la" ]] <- ( dMu[[ "raw.GC.la" ]] + dMu.GC ) * 2 / ( (1 + dMu.GC) * (1 - dMu.GC) ) ;

        results[[ "AT.le" ]] <- ( dMu[[ "raw.AT.le" ]] - dMu.AT ) * 2 / ( (1 + dMu.AT) * (1 - dMu.AT) ) ;
        results[[ "AT.la" ]] <- ( dMu[[ "raw.AT.la" ]] + dMu.AT ) * 2 / ( (1 + dMu.AT) * (1 - dMu.AT) ) ;
...
taiStats <- function( data, dat, dMuIO, z4s, model = "Model07" ){
    ...
    for( i in 1:max( data[, "quantile"] ) ){
        ...
        ## -- zaa --
        if( model == "Model07" ){
            l[["zAA.2s"]] <- zAAcalcM07(  dMu.xs= l[["dMu.2s"]], dMu.4s= l[["dMu.4s"]], dMu.io= dMuIO, z.4s = z4s );
            l[["zAA.1s"]] <- zAAcalcM07(  dMu.xs= l[["dMu.1s"]], dMu.4s= l[["dMu.4s"]], dMu.io= dMuIO, z.4s = z4s );
        } else if( model == "Chen" ){
...
dMuCalc4PrecalcSkews <- function( type, data = tai10, data2 = dat, iodata = "new", model = "Model07" ){
    results <- list();
